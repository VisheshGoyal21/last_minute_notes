import React from 'react';

const Footer = () => {
	return (
		<div className="grid grid-cols-3 gap-20 text-gray-200 bg-gray-900 px-8 py-12">
			<div className="md:col-span-1 col-span-3">
				<h1 className="text-2xl underline font-heading">About Us</h1>
				<br />
				<p>
					We are College Friends Pursuing B-Tech in Computer Science and Engineering.
					We have develped this Web Application to make it easier for students of the
					university to find good quality notes irrespective of their branch and
					semester
					<br />
					This build a strong Community where study material can be shared easily.
				</p>
			</div>
			<div className="col-span-3 md:col-span-2">
				<h1 className="text-2xl underline font-heading">Contact Us</h1>
				<br />
				<div className="grid grid-cols-2 text-gray-300 justify-center">
					<div className="">
						<span className="font-semibold text-xl">
							<p>Manish Motwani</p>
							<br />
							<a href="https://www.linkedin.com/in/manish-motwani-76729a148/">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 28.87 28.87"
									width="40px"
									className="inline-block mx-2">
									<g data-name="Layer 2">
										<g data-name="Layer 1">
											<rect
												width="28.87"
												height="28.87"
												fill="#0b86ca"
												rx="6.48"
												ry="6.48"
											/>
											<path
												fill="#fff"
												d="M8 12h3v9.68H8zm1.53-4.81a1.74 1.74 0 11-1.74 1.75 1.74 1.74 0 011.74-1.75M12.92 12h2.89v1.32a3.16 3.16 0 012.85-1.56c3 0 3.61 2 3.61 4.61v5.31h-3V17c0-1.12 0-2.57-1.56-2.57s-1.8 1.22-1.8 2.48v4.79h-3z"
											/>
										</g>
									</g>
								</svg>
							</a>
							<a href="https://github.com/manishmotwani2002">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									width="40px"
									height="48"
									fill="none"
									viewBox="0 0 48 48"
									className="inline-block mx-2">
									<rect width="48" height="48" fill="#000" rx="24" />
									<path
										fill="#fff"
										fill-rule="evenodd"
										d="M31.4225 46.8287C29.0849 47.589 26.5901 48 24 48C21.4081 48 18.9118 47.5884 16.5728 46.8272C17.6533 46.9567 18.0525 46.2532 18.0525 45.6458C18.0525 45.3814 18.048 44.915 18.0419 44.2911C18.035 43.5692 18.0259 42.6364 18.0195 41.5615C11.343 43.0129 9.9345 38.3418 9.9345 38.3418C8.844 35.568 7.2705 34.8294 7.2705 34.8294C5.091 33.3388 7.4355 33.369 7.4355 33.369C9.843 33.5387 11.1105 35.8442 11.1105 35.8442C13.2525 39.5144 16.728 38.4547 18.096 37.8391C18.3135 36.2871 18.9345 35.2286 19.62 34.6283C14.2905 34.022 8.688 31.9625 8.688 22.7597C8.688 20.1373 9.6225 17.994 11.1585 16.3142C10.911 15.7065 10.0875 13.2657 11.3925 9.95888C11.3925 9.95888 13.4085 9.31336 17.9925 12.4206C19.908 11.8876 21.96 11.6222 24.0015 11.6114C26.04 11.6218 28.0935 11.8876 30.0105 12.4206C34.5915 9.31336 36.603 9.95888 36.603 9.95888C37.9125 13.2657 37.089 15.7065 36.8415 16.3142C38.3805 17.994 39.309 20.1373 39.309 22.7597C39.309 31.9849 33.6975 34.0161 28.3515 34.6104C29.2125 35.3519 29.9805 36.8168 29.9805 39.058C29.9805 41.2049 29.9671 43.0739 29.9582 44.3125C29.9538 44.9261 29.9505 45.385 29.9505 45.6462C29.9505 46.2564 30.3401 46.9613 31.4225 46.8287Z"
										clip-rule="evenodd"
									/>
								</svg>
							</a>
							<a href="https://www.youtube.com/channel/UCEp5dV9ZPf1kvrOA7SAN1rw">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 28.87 28.87"
									width="40px"
									className="inline-block mx-2">
									<g data-name="Layer 2">
										<g data-name="Layer 1">
											<rect
												width="28.87"
												height="28.87"
												fill="#fd3832"
												rx="6.48"
												ry="6.48"
											/>
											<path
												fill="#fff"
												fill-rule="evenodd"
												d="M8 19.77a1.88 1.88 0 0 1-1.24-1.21c-.54-1.48-.7-7.66.34-8.88A2 2 0 0 1 8.46 9c2.79-.3 11.41-.26 12.4.1a1.94 1.94 0 0 1 1.22 1.17c.59 1.53.61 7.09-.08 8.56a1.89 1.89 0 0 1-.87.88c-1.04.52-11.75.51-13.13.06zm4.43-2.9l5-2.6-5-2.62z"
											/>
										</g>
									</g>
								</svg>
							</a>
						</span>
					</div>
					<div>
						<span className="font-semibold text-xl">
							<p className="ml-4">Lavesh Garg</p>
							<br />
							<a href="https://www.linkedin.com/in/lavesh-garg-7249881a7/">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 28.87 28.87"
									width="40px"
									className="inline-block mx-2">
									<g data-name="Layer 2">
										<g data-name="Layer 1">
											<rect
												width="28.87"
												height="28.87"
												fill="#0b86ca"
												rx="6.48"
												ry="6.48"
											/>
											<path
												fill="#fff"
												d="M8 12h3v9.68H8zm1.53-4.81a1.74 1.74 0 11-1.74 1.75 1.74 1.74 0 011.74-1.75M12.92 12h2.89v1.32a3.16 3.16 0 012.85-1.56c3 0 3.61 2 3.61 4.61v5.31h-3V17c0-1.12 0-2.57-1.56-2.57s-1.8 1.22-1.8 2.48v4.79h-3z"
											/>
										</g>
									</g>
								</svg>
							</a>
							<a href="https://github.com/laveshgarg80">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									width="40px"
									height="48"
									fill="none"
									viewBox="0 0 48 48"
									className="inline-block mx-2">
									<rect width="48" height="48" fill="#000" rx="24" />
									<path
										fill="#fff"
										fill-rule="evenodd"
										d="M31.4225 46.8287C29.0849 47.589 26.5901 48 24 48C21.4081 48 18.9118 47.5884 16.5728 46.8272C17.6533 46.9567 18.0525 46.2532 18.0525 45.6458C18.0525 45.3814 18.048 44.915 18.0419 44.2911C18.035 43.5692 18.0259 42.6364 18.0195 41.5615C11.343 43.0129 9.9345 38.3418 9.9345 38.3418C8.844 35.568 7.2705 34.8294 7.2705 34.8294C5.091 33.3388 7.4355 33.369 7.4355 33.369C9.843 33.5387 11.1105 35.8442 11.1105 35.8442C13.2525 39.5144 16.728 38.4547 18.096 37.8391C18.3135 36.2871 18.9345 35.2286 19.62 34.6283C14.2905 34.022 8.688 31.9625 8.688 22.7597C8.688 20.1373 9.6225 17.994 11.1585 16.3142C10.911 15.7065 10.0875 13.2657 11.3925 9.95888C11.3925 9.95888 13.4085 9.31336 17.9925 12.4206C19.908 11.8876 21.96 11.6222 24.0015 11.6114C26.04 11.6218 28.0935 11.8876 30.0105 12.4206C34.5915 9.31336 36.603 9.95888 36.603 9.95888C37.9125 13.2657 37.089 15.7065 36.8415 16.3142C38.3805 17.994 39.309 20.1373 39.309 22.7597C39.309 31.9849 33.6975 34.0161 28.3515 34.6104C29.2125 35.3519 29.9805 36.8168 29.9805 39.058C29.9805 41.2049 29.9671 43.0739 29.9582 44.3125C29.9538 44.9261 29.9505 45.385 29.9505 45.6462C29.9505 46.2564 30.3401 46.9613 31.4225 46.8287Z"
										clip-rule="evenodd"
									/>
								</svg>
							</a>
							<a href="https://www.youtube.com/channel/UCEp5dV9ZPf1kvrOA7SAN1rw">
								<svg
									xmlns="http://www.w3.org/2000/svg"
									viewBox="0 0 28.87 28.87"
									width="40px"
									className="inline-block mx-2">
									<g data-name="Layer 2">
										<g data-name="Layer 1">
											<rect
												width="28.87"
												height="28.87"
												fill="#fd3832"
												rx="6.48"
												ry="6.48"
											/>
											<path
												fill="#fff"
												fill-rule="evenodd"
												d="M8 19.77a1.88 1.88 0 0 1-1.24-1.21c-.54-1.48-.7-7.66.34-8.88A2 2 0 0 1 8.46 9c2.79-.3 11.41-.26 12.4.1a1.94 1.94 0 0 1 1.22 1.17c.59 1.53.61 7.09-.08 8.56a1.89 1.89 0 0 1-.87.88c-1.04.52-11.75.51-13.13.06zm4.43-2.9l5-2.6-5-2.62z"
											/>
										</g>
									</g>
								</svg>
							</a>
						</span>
					</div>
				</div>
			</div>
		</div>
	);
};

export default Footer;
